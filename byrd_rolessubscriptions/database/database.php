...
if (!class_exists('bDatabase')){ class bDatabase {
	...
	*/
	function &getInstance( $options	= array() )
	{
		...
		if (empty($instances[$signature]))
		{
			...
			if (file_exists($path)) {
				...
				trigger_error('Unable to load Database Driver:'.$driver);
				return false;
			}
			...
			{
				trigger_error('Unable to connect to the database:'.$error);
				return false;
			}
	...
	{
		eImport('libraries.filesystem.folder');
	...
	*/
	...
	{
		require_once EPATH_LIB.DS.'database'.DS.'database'.DS.'recordset.php';

		$query = trim( $query );
		$this->setQuery( $query );
		if (eregi( '^select', $query )) {
			$result = $this->loadRowList();
			return new eRecordSet( $result );
		} else {
			$result = $this->query();
			if ($result === false) {
				return false;
			} else {
				return new eRecordSet( array() );
			}
		}
	}
