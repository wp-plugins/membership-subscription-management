 ...
 * @since		1.0
 */
class eDatabaseODBC extends eDatabase
{
	/**
	...
	*/
	function __construct( $options )
	{
		...
		$driver	= array(
			...
			3 => '{Microsoft Excel Driver (*.xls)}',
			4 => '{SQL Server}',
			5 => '{Adaptive Server Anywhere 8.0}'
		);
		$provider	= array(
	...
	function query()
	{
		...
		if (!$this->_cursor)
		{
			...
			if ($this->_debug) {
				JError::raiseError(500, 'eDatabaseMySQL::query: '.$this->_errorNum.' - '.$this->_errorMsg );
			}
			return false;
		}
		return $this->_cursor;
